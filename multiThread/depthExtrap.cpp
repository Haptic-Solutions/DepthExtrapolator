            ...
            for(int Tx=LLX+Xsq_wdth;Tx<x+Pix_End;Tx+=CPUthreads){
                ...
                if(!PixSkip[cord(Tx,y)]&&Tx!=(width/2)){
                    if(reduxMatch(x,Tx,y)){
                        ...
                        int lowestThrd = 0;
                        for(int thrdNum=0;thrdNum<CPUthreads;thrdNum++){
                            if(PX_CPU_Match[thrdNum]<PX_Match){
                                PX_Match=PX_CPU_Match[thrdNum];
                                lowestThrd=thrdNum;
                            }
                        }
                        if(PX_Match<lowest_Diff&&PX_Match>=0){
                            lowest_Diff=PX_Match;
                            lowest_Tx=Tx+lowestThrd;
                            multi_point=0;
                        }
...
///Grid comparator.
void C_gridProc::gridComp(int x, int Tx, int y, int*PX_CPU_Match, int thrdNum){
    int PX_Match=0;
    int LctstCount=0;
